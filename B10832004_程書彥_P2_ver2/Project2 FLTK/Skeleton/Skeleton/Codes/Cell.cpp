...
Clip_To_Cell(float& xs, float& ys,
    float& xe, float& ye, const float buffer, int error)
    //=======================================================================
{
    ...
    float correctValue;
    if (error == 1)
        correctValue = 1.0e-6f;
    else
        correctValue = 1.0e-2f; // 1.0e-4f
    ...
    if (min_crossing < 1.0) {
        if (edges[min_cross_edge]->opaque) {
            min_crossing -= correctValue; // Make sure we stay inside. 1.0e-4f
            if (min_crossing < 0.0)
                min_crossing = 0.0f;
        ...
        {
            min_crossing += correctValue; // Make sure the new point is outside. 1.0e-4f
            if (min_crossing > 1.0)
                min_crossing = 1.0f;
