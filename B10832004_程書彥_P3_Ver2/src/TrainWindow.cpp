...
//========================================================================
{
	...
	//#####################################################################
	for (int j = 0; j < trainView->m_pTrack->trainNum; j++) {

		auto currentTrain = &(trainView->m_pTrack->trainU[j]);
		if (arcLength->value() == 0) {

			*currentTrain += dir * ((float)speed->value() * 0.1);
			if (*currentTrain > trainView->m_pTrack->points.size()) {

				*currentTrain -= trainView->m_pTrack->points.size();
			}
			else if (*currentTrain < 0) {
			...
			float distanceNeedToMove = dir * (float)speed->value() * 1.0;
			float* trainUPtr = &(*currentTrain);
			float* arcLength = trainView->m_pTrack->arcLength;
			float dividedLine = trainView->m_pTrack->DIVIDED_LINE;
			while (true) {

				int nextLengthIndex = (int)floor(dividedLine * (*(trainUPtr)));
				while (nextLengthIndex >= trainView->m_pTrack->points.size() * dividedLine) {

					nextLengthIndex -= trainView->m_pTrack->points.size() * dividedLine;
				}
				if (arcLength[nextLengthIndex] >= distanceNeedToMove) {

					*currentTrain += 1.0 / dividedLine;
					break;
				}
				else {

					distanceNeedToMove -= arcLength[nextLengthIndex];
					*currentTrain += 1.0 / dividedLine;
				}
			}
			while (*currentTrain >= trainView->m_pTrack->points.size()) {
